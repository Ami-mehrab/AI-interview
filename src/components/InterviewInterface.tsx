...
import { Mic, MicOff, Play, Square, Loader2, User, Bot, MessageSquare, Plus, List, Trash2, CheckCircle2, Sparkles, LogIn, Upload, FileSpreadsheet, ExternalLink, Share2, Settings, Info, Edit3, Save, X, ChevronUp, ChevronDown, Copy, FileText, Briefcase, ArrowLeft, Clock, AlertCircle, Trophy } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import * as XLSX from 'xlsx';
import { float32ToPcm, base64ToUint8Array, uint8ArrayToBase64 } from '../lib/audio-utils';
import { auth, db } from '../lib/firebase';
import { signInWithPopup, GoogleAuthProvider, onAuthStateChanged, User as FirebaseUser, signOut, signInWithCustomToken } from 'firebase/auth';
import { collection, addDoc, query, where, onSnapshot, serverTimestamp, doc, updateDoc, setDoc, arrayUnion, getDocs, deleteDoc, getDoc } from 'firebase/firestore';
...
interface Candidate {
  ...
  status: 'uploaded' | 'questions_generated' | 'approved' | 'interview_started' | 'interview_completed' | 'completed';
...
interface Interview {
  ...
  job_description: string;
  approved_questions: string[];
  transcript: { speaker: 'user' | 'assistant'; text: string; timestamp: string }[];
  transcript_text: string;
  evaluation_json: any;
  evaluation?: any;
  score?: number;
...
export default function InterviewInterface() {
  ...
  const isRecruiter = (u: FirebaseUser | null) => u?.email === 'mahrabbappy33@gmail.com';
  
  const [view, setView] = useState<'recruiter' | 'candidate' | 'candidates-list' | 'dashboard'>(() => {
  ...
  const [newJd, setNewJd] = useState({ role: '', description: '', generatedQuestions: [] as string[] });
  const [newCandidate, setNewCandidate] = useState({ candidate_name: '', candidate_email: '', applied_role: '', job_description: '', generatedQuestions: [] as string[] });
  ...
  const [hasPermissions, setHasPermissions] = useState(localStorage.getItem('interview_permissions_granted') === 'true');
  ...
  const currentTurnRef = useRef<{ speaker: 'user' | 'assistant', text: string } | null>(null);
    ...
    if (currentInterviewId && transcript.length > 0) {
      const timer = setTimeout(() => {
        updateDoc(doc(db, 'candidates', currentInterviewId), {
          ...
          transcript_text: transcript.map(e => `${e.speaker === 'assistant' ? 'AI' : 'Candidate'}: ${e.text}`).join('\n\n'),
          ...
          if (user && user.email) {
            ...
            try {
              ...
              if (interviewSnap.exists()) {
                ...
                if (fullData.candidate_email?.toLowerCase() === user.email.toLowerCase() || isRecruiter(user)) {
                  ...
                  if (fullData.transcript) {
                    ...
                    const assistantTurns = fullData.transcript.filter((t: any) => t.speaker === 'assistant').length;
                  ...
                  // Email mismatch
                  setJoinError(`Unauthorized: This interview is reserved for ${fullData.candidate_email}`);
                  setIsEmailVerified(false);
                }
              }
            } catch (e) {
              // Permission denied or other error
              if (!isRecruiter(user) && metaData.candidate_email && user.email.toLowerCase() !== metaData.candidate_email.toLowerCase()) {
                setJoinError(`Unauthorized: This interview is reserved for ${metaData.candidate_email}`);
  ...
  useEffect(() => {
    ...
    const qCandidates = query(collection(db, 'candidates'), where('createdBy', '==', user?.uid));
    const unsubscribeCandidates = onSnapshot(qCandidates, (snapshot) => {
      const loadedCandidates = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Candidate));
  ...
  const login = async () => {
    try {
      ...
      if (error.code === 'auth/configuration-not-found') {
        showToast("Firebase Auth is not enabled. Please enable Google Sign-In in your Firebase Console.", "error");
      } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (interviewSnap.exists()) {
          ...
          if (fullData.transcript) {
            setTranscript(fullData.transcript);
            setQuestionCount(fullData.transcript.filter(t => t.speaker === 'assistant').length);
          }
          if (fullData.status === 'completed') setIsCompleted(true);
        }
        showToast("Welcome to the interview!", "success");
      } else {
        const errorMsg = data.details ? `${data.error}: ${data.details}` : (data.error || "Verification failed");
      ...
      if (error instanceof Error) {
        ...
        setJoinError("An error occurred. Please check your connection and try again.");
      }
    } finally {
      setIsJoining(false);
    }
...
Return the questions as a JSON array of strings.`,
  ...
  const deleteJD = async (id: string) => {
    try {
      ...
      handleFirestoreError(error, OperationType.DELETE, `jobDescriptions/${id}`);
    }
  };

  const generateCandidateQuestions = async () => {
    if (!newCandidate.candidate_name || !newCandidate.applied_role || !newCandidate.job_description) return;
    setIsGenerating(true);
    try {
      const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
      const response = await ai.models.generateContent({
...
Return the questions as a JSON array of strings.`,
  ...
  const addCandidate = async () => {
    if (!user || !newCandidate.candidate_name || !newCandidate.applied_role) return;
    try {
      await addDoc(collection(db, 'candidates'), {
        ...
        status: newCandidate.generatedQuestions.length > 0 ? 'questions_generated' : 'uploaded',
        interview_link: '',
        createdBy: user.uid,
        created_at: serverTimestamp(),
        updated_at: serverTimestamp(),
      });
      setNewCandidate({ candidate_name: '', candidate_email: '', applied_role: '', job_description: '', generatedQuestions: [] });
  ...
  useEffect(() => {
    if (selectedCandidate) {
      setEditableQuestions(selectedCandidate.approved_questions || selectedCandidate.generated_questions || []);
    } else {
  ...
  const saveQuestions = async () => {
    ...
    try {
      await setDoc(doc(db, 'candidates', selectedCandidate.id), {
        approved_questions: editableQuestions.filter(q => q.trim() !== ""), // Prevent saving empty blanks
        questions_count: editableQuestions.filter(q => q.trim() !== "").length,
        updated_at: serverTimestamp()
      }, { merge: true });
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        // Validate columns
        const requiredColumns = ['candidate_name', 'applied_role', 'job_description', 'candidate_email'];
        ...
        const batch = data.map(async (row: any) => {
          ...
          // Ignore empty rows
          if (!candidate_name && !candidate_email && !applied_role && !job_description) return;
        ...
        showToast("Failed to upload candidates. Ensure the file is valid.", "error");
      } finally {
        setIsUploading(false);
        // Reset input
        e.target.value = '';
...
Return the questions as a JSON array of strings.`,
  ...
  const approveQuestions = async (candidateId: string) => {
    ...
    if (!candidate.approved_questions || candidate.approved_questions.length === 0) {
  ...
  const startCandidateInterview = async (candidateId: string) => {
    ...
    if (!candidate.questions_approved || !candidate.approved_questions || candidate.approved_questions.length === 0) {
  ...
  const saveEditedQuestions = async (candidateId: string, questions: string[]) => {
    try {
      await updateDoc(doc(db, 'candidates', candidateId), {
        approved_questions: questions,
        questions_count: questions.length,
  ...
  const [baseShareUrl, setBaseShareUrl] = useState<string>(() => {
    return localStorage.getItem('genie_base_share_url') || window.location.origin;
  });
  ...
  useEffect(() => {
    if (!selectedCandidate || !selectedCandidate.window_expires_at || selectedCandidate.session_status !== 'open') {
      setTimeLeft(null);
      return;
    }
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, 'candidates', currentInterviewId), (snapshot) => {
      if (snapshot.exists()) {
        const data = snapshot.data() as Interview;
        if (data.transcript && JSON.stringify(data.transcript) !== JSON.stringify(transcript)) {
          // Map stored speaker to state speaker
          setTranscript(data.transcript.map(t => ({ speaker: t.speaker, text: t.text })));
        }
      }
    }, (error) => handleFirestoreError(error, OperationType.GET, `candidates/${currentInterviewId}`));
  ...
  const startLiveInterview = async () => {
    try {
      ...
      const interviewRef = interviewId ? doc(db, 'candidates', interviewId) : null;
      const interviewSnap = interviewRef ? await getDoc(interviewRef) : null;

      let targetQuestions = selectedCandidate ? (selectedCandidate.approved_questions || selectedCandidate.generated_questions) : [];
      let targetRole = selectedCandidate ? selectedCandidate.applied_role : "Candidate";
      let targetJd = selectedCandidate ? selectedCandidate.job_description : "";
      let targetEmail = selectedCandidate ? selectedCandidate.candidate_email : "";
      let targetName = candidateName || selectedCandidate?.candidate_name || "Candidate";
      ...
      if (!interviewId || !interviewSnap?.exists()) {
        ...
        if (data.status === 'not_started') {
          try {
            ...
            console.error("Failed to update interview/candidate status:", error);
          }
        }
      }
...
- No formatting.`;
        ...
        // Trigger the AI to start the interview according to its system instructions
          ...
          onmessage: async (message: LiveServerMessage) => {
            ...
            if (serverContent) {
              ...
              if (userText) {
                if (currentTurnRef.current && currentTurnRef.current.speaker === 'assistant') {
                  finalizeTurn();
                }
                if (!currentTurnRef.current || currentTurnRef.current.speaker !== 'user') {
                ...
                if (last && last.speaker === 'user') {
                  ...
                  transcriptRef.current.push({ speaker: 'user', text: userText, timestamp: new Date().toISOString() });
                }
              ...
              if (aiText) {
                ...
                if (!currentTurnRef.current || currentTurnRef.current.speaker !== 'assistant') {
                  currentTurnRef.current = { speaker: 'assistant', text: aiText };
                } else {
                ...
                if (last && last.speaker === 'assistant') {
                  ...
                  transcriptRef.current.push({ speaker: 'assistant', text: aiText, timestamp: new Date().toISOString() });
                }
                setTranscript([...transcriptRef.current]);
              }
  ...
  const finalizeTurn = async () => {
    ...
    const turn = { ...currentTurnRef.current, timestamp: new Date().toISOString() };
    ...
    try {
      ...
      if (speaker === 'assistant') {
        setQuestionCount(prev => {
          const next = prev + 1;
          const totalQuestions = selectedCandidate?.approved_questions?.length || 10;
  ...
  const stopInterview = async () => {
    ...
    if (currentInterviewId) {
      ...
      try {
        await setDoc(doc(db, 'candidates', currentInterviewId), { 
          transcript: transcriptRef.current,
          transcript_text: transcriptRef.current.map(e => `${e.speaker === 'assistant' ? 'AI' : 'Candidate'}: ${e.text}`).join('\n\n'),
          status: 'completed',
  ...
  const generateEvaluation = async (candidateId: string) => {
    ...
    try {
      ...
      if (!docSnap.exists()) {
        console.error("Evaluation failed: Candidate document missing for ID", candidateId);
        throw new Error("Candidate document missing.");
      }
      
      const freshData = docSnap.data();
      const transcriptData = freshData.transcript_text || 
        (Array.isArray(freshData.transcript) 
          ? freshData.transcript.map((e: any) => `${e.speaker === 'assistant' ? 'AI' : 'Candidate'}: ${e.text}`).join('\n\n') 
          : "");

      if (!transcriptData || transcriptData.trim() === "") {
        console.error("Evaluation failed: Empty transcript for candidate", candidateId);
      ...
      showToast(error instanceof Error ? error.message : "Failed to generate AI evaluation. Please try again.", "error");
    } finally {
      setIsEvaluating(false);
    }
  };
...
- If evidence for a category is weak or the response is incomplete, score conservatively.`,
          ...
          responseSchema: {
            ...
            properties: {
              ...
              scores: {
                ...
                required: ["technical_accuracy", "problem_solving", "communication_skills", "confidence", "clarity", "relevance_of_answers", "role_fit", "overall_performance"]
              },
              ...
              question_by_question_analysis: {
                ...
                items: {
                  ...
                  required: ["question", "answer_quality", "key_observation", "score"]
                }
              },
              hiring_recommendation: { type: Type.STRING },
              recommended_next_step: { type: Type.STRING }
            },
            required: ["candidate_name", "candidate_email", "applied_role", "overall_summary", "scores", "strengths", "weaknesses", "question_by_question_analysis", "hiring_recommendation", "recommended_next_step"]
          }
        }
      });
  ...
  const fetchEvaluation = async (candidateId: string) => {
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        const evaluation = interviewData.evaluation || interviewData.evaluation_json;
        if (evaluation) {
  ...
  const requestMediaAccess = async () => {
    ...
    try {
      ...
      const msg = "Camera and microphone access are required for the interview. Please enable them in your browser settings.";
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-slate-950 text-white p-6">
          ...
          >
            ...
            <div className="space-y-2">
              <h1 className="text-3xl font-bold tracking-tight">InterviewGenie</h1>
              <p className="text-slate-400">Sign in to manage JDs and conduct interviews.</p>
            </div>
            <button
              onClick={login}
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-slate-950 text-white p-6">
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-red-400 text-sm font-bold">Invalid: You are not admin.</p>
              <p className="text-slate-400 text-xs">Only the authorized recruiter can access the dashboard. You are signed in as <span className="text-white font-bold">{user.email}</span>.</p>
            </div>
            <button
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-950 text-white p-6 font-sans">
      <div className="w-full max-w-screen-2xl bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row h-auto md:h-[800px]">
          ...
          <div className="w-full md:w-80 border-t border-slate-800 md:border-r md:border-t-0 flex flex-col bg-slate-900/80">
            <div className="p-6 border-b border-slate-800 flex items-center justify-between">
              ...
              <div className="flex gap-1">
                ...
                <label className={`flex items-center gap-2 px-3 py-1.5 bg-blue-600 hover:bg-blue-500 rounded-lg text-[10px] font-bold text-white transition-all cursor-pointer ${isUploading ? 'opacity-50 cursor-not-allowed' : ''}`}>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {candidates.map(c => (
                <div 
                  key={c.id}
                  onClick={() => { setSelectedCandidateId(c.id); setView('dashboard'); }}
                ...
                >
                  <div className="flex justify-between items-start gap-3">
                    <div className="min-w-0">
                      <p className="font-semibold text-sm truncate pr-2">{c.candidate_name}</p>
                      <p className="text-[10px] text-slate-400 truncate">{c.applied_role}</p>
                    </div>
                    <button 
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-2 text-[10px] text-slate-500">
                    <span className="px-2 py-1 rounded-full bg-slate-800/80 border border-slate-700">{(c.approved_questions || c.generated_questions || []).length} Questions</span>
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
            ...
            <div className="p-4 border-t border-slate-800">
              <div className="flex items-center gap-3 px-3 py-2 bg-slate-800/50 rounded-xl">
                ...
                <div className="min-w-0">
                  ...
                  <button onClick={() => signOut(auth)} className="text-[10px] text-slate-500 hover:text-white">Sign Out</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Main Area */}
        <div className={`flex-1 flex flex-col overflow-hidden ${(!isRecruiter(user) || view === 'candidate') ? 'w-full' : ''}`}>
          {view === 'recruiter' ? (
            <div className="p-12 flex flex-col h-full overflow-y-auto">
              {/* JD Form */}
              <div className="max-w-xl mx-auto w-full space-y-8">
                <div className="space-y-2">
                  <h2 className="text-2xl font-bold">Add New Job Description</h2>
                  <p className="text-slate-400 text-sm">Genie will automatically generate 10 relevant interview questions.</p>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  {newJd.generatedQuestions.length === 0 ? (
                    <button
                      ...
                      disabled={isGenerating || !newJd.role || !newJd.description}
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-500 uppercase">Review & Edit Questions</label>
                        <div className="space-y-2">
                          {newJd.generatedQuestions.map((q, i) => (
                            <input 
                              ...
                              onChange={e => {
                                ...
                                setNewJd({ ...newJd, generatedQuestions: updated });
                              }}
                  ...
                  <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="p-8 border-b border-slate-800 bg-slate-900/50 shrink-0">
                      <div className="grid gap-4 md:grid-cols-[1.2fr_0.8fr] items-start max-w-5xl mx-auto w-full">
                        <div className="space-y-2">
                          <h3 className="text-2xl font-bold tracking-tight">{selectedCandidate.candidate_name}</h3>
                        ...
                        <div className="flex flex-col items-start gap-2 sm:items-end">
                          <span className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest border ${
                            selectedCandidate.status === 'interview_completed' ? 'bg-green-500/10 text-green-400 border-green-500/20' :
                            selectedCandidate.status === 'completed' ? 'bg-green-500/10 text-green-400 border-green-500/20' :
                            selectedCandidate.status === 'interview_started' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                            selectedCandidate.status === 'approved' ? 'bg-purple-500/10 text-purple-400 border-purple-500/20' :
                            selectedCandidate.status === 'questions_generated' ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' :
                      ...
                      {selectedCandidate.interview_id && (
                        <div className="mt-4 max-w-4xl mx-auto w-full flex items-center justify-between bg-slate-800/20 p-4 rounded-2xl border border-slate-800/50">
                          <div className="flex items-center gap-2 text-xs">
                            <span className="text-slate-500 uppercase font-bold tracking-wider">Session Status:</span>
                            <span className={`font-bold uppercase tracking-widest ${
                              selectedCandidate.session_status === 'open' ? 'text-green-400' :
                              selectedCandidate.session_status === 'in-progress' ? 'text-blue-400' :
                              selectedCandidate.session_status === 'expired' ? 'text-red-400' :
                              'text-slate-400'
                            }`}>
                          ...
                          {timeLeft !== null && timeLeft > 0 && (
                            <div className="flex items-center gap-2 px-3 py-1 bg-blue-500/10 rounded-full border border-blue-500/20">
                              ...
                              <span className="text-[10px] font-bold text-blue-400">Joining Window: {timeLeft}s</span>
                            </div>
                          )}
                          {timeLeft === 0 && selectedCandidate.session_status === 'open' && (
                            <div className="flex items-center gap-2 px-3 py-1 bg-red-500/10 rounded-full border border-red-500/20">
                              <AlertCircle className="w-3 h-3 text-red-400" />
                              <span className="text-[10px] font-bold text-red-400">Window Expired</span>
                            </div>
                          )}
                        </div>
                      )}
                      ...
                      <div className="mt-8 grid gap-3 max-w-5xl mx-auto w-full sm:grid-cols-2 xl:grid-cols-3">
                        {selectedCandidate.status === 'uploaded' && (
                          <button 
                            onClick={() => generateQuestionsForCandidate(selectedCandidate.id)}
                            disabled={loadingAction?.id === selectedCandidate.id}
                          ...
                          >
                            {loadingAction?.id === selectedCandidate.id && loadingAction.action === 'generating' ? <Loader2 className="w-5 h-5 animate-spin" /> : <Sparkles className="w-5 h-5" />}
                            Generate Questions
                          </button>
                        )}

                        {(selectedCandidate.status === 'questions_generated' || selectedCandidate.status === 'approved') && (
                          <div className="w-full flex gap-3 flex-wrap">
                            <button 
                              onClick={() => regenerateQuestionsForCandidate(selectedCandidate.id)}
                              disabled={loadingAction?.id === selectedCandidate.id}
                            ...
                            >
                              {loadingAction?.id === selectedCandidate.id && loadingAction.action === 'regenerating' ? <Loader2 className="w-5 h-5 animate-spin" /> : <Sparkles className="w-5 h-5" />}
                              Regenerate
                            </button>
                            {selectedCandidate.status === 'questions_generated' && (
                              <button 
                                onClick={() => approveQuestions(selectedCandidate.id)}
                                disabled={loadingAction?.id === selectedCandidate.id}
                              ...
                              >
                                {loadingAction?.id === selectedCandidate.id && loadingAction.action === 'approving' ? <Loader2 className="w-5 h-5 animate-spin" /> : <CheckCircle2 className="w-5 h-5" />}
                                Approve Questions
                              </button>
                            )}
                          </div>
                        ...
                        {selectedCandidate.status === 'approved' && (
                          <div className="w-full flex gap-3">
                            <button 
                              onClick={() => startCandidateInterview(selectedCandidate.id)}
                              disabled={loadingAction?.id === selectedCandidate.id}
                            ...
                            >
                              {loadingAction?.id === selectedCandidate.id && loadingAction.action === 'starting' ? <Loader2 className="w-5 h-5 animate-spin" /> : <Play className="w-5 h-5" />}
                              Start Candidate Session (48h)
                            </button>
                            <button 
                              onClick={() => {
                                const link = getInterviewLink(selectedCandidate);
                                navigator.clipboard.writeText(link);
                        ...
                        {selectedCandidate.status === 'interview_started' && (
                          <div className="w-full flex flex-col gap-3">
                            <div className="flex gap-3">
                              ...
                              >
                                {loadingAction?.id === selectedCandidate.id && loadingAction.action === 'starting' ? <Loader2 className="w-5 h-5 animate-spin" /> : <Clock className="w-5 h-5" />}
                            ...
                            <button 
                              onClick={async () => {
                                try {
                                  setLoadingAction({ id: selectedCandidate.id, action: 'forcing' });
                                  await setDoc(doc(db, 'candidates', selectedCandidate.id), { 
                                    status: 'completed',
                                    session_status: 'completed',
                                    updated_at: serverTimestamp() 
                                  }, { merge: true });
                                  showToast("Session forced to complete", "success");
                                } catch (error) {
                                  console.error("Force complete failed:", error);
                                  showToast("Failed to force complete session", "error");
                                } finally {
                                  setLoadingAction(null);
                                }
                              }}
                            ...
                            >
                              {loadingAction?.id === selectedCandidate.id && loadingAction.action === 'forcing' ? <Loader2 className="w-3 h-3 animate-spin" /> : <AlertCircle className="w-3 h-3" />}
                              Force Complete Session (Admin Override)
                            </button>
                          </div>
                        )}

                        {(selectedCandidate?.status === 'interview_completed' || selectedCandidate?.status === 'completed') && (
                          <div className="w-full space-y-4">
                            {!selectedCandidate.evaluation ? (
                              <button 
                                onClick={() => generateEvaluation(selectedCandidate.id!)}
                                disabled={isEvaluating}
                                className="w-full flex items-center justify-center gap-3 px-8 py-5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 rounded-2xl font-bold transition-all shadow-2xl shadow-blue-600/40 disabled:opacity-50 transform hover:scale-[1.02] active:scale-[0.98]"
                              >
                                {isEvaluating ? <Loader2 className="w-6 h-6 animate-spin" /> : <Sparkles className="w-6 h-6" />} 
                                <span className="text-lg">Generate AI Evaluation</span>
                              </button>
                            ) : selectedCandidate.evaluation.error ? (
                              <div className="w-full p-8 bg-slate-900/80 rounded-[32px] border border-slate-800 shadow-2xl text-center">
                                <AlertCircle className="w-12 h-12 text-amber-500 mx-auto mb-4" />
                                <h4 className="text-lg font-bold text-white mb-2">Evaluation Unavailable</h4>
                                <p className="text-slate-300">{selectedCandidate.evaluation.error}</p>
                              </div>
                            ) : (
                              <div className="w-full space-y-6 p-8 bg-slate-900/80 rounded-[32px] border border-slate-800 shadow-2xl">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-4">
                                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner ${
                                      selectedCandidate?.score! >= 8 ? 'bg-emerald-500/10 text-emerald-500' :
                                      selectedCandidate?.score! >= 5 ? 'bg-amber-500/10 text-amber-500' :
                                    ...
                                    <div>
                                      ...
                                      <div className="flex items-center gap-2">
                                        <span className={`text-xs font-bold uppercase tracking-widest ${
                                          selectedCandidate?.score! >= 8 ? 'text-emerald-500' :
                                          selectedCandidate?.score! >= 5 ? 'text-amber-500' :
                                          'text-red-500'
                                        }`}>
                                          {selectedCandidate?.score! >= 8 ? 'Pass' :
                                           selectedCandidate?.score! >= 5 ? 'Maybe' :
                                           'Reject'}
                                        </span>
                                      </div>
                                    </div>
                                  ...
                                  <div className="flex flex-col items-end">
                                    ...
                                    <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mt-1">Score</p>
                                  </div>
                                </div>

                                <div className="space-y-6">
                                  <div className="p-5 bg-slate-800/30 rounded-2xl border border-slate-700/30">
                                    <h5 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">Evaluation Summary</h5>
                                    <p className="text-sm text-slate-300 leading-relaxed font-medium">{selectedCandidate.evaluation.summary}</p>
                                  </div>
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-4">
                                      <div className="flex items-center gap-2">
                                        ...
                                        <h5 className="text-[10px] font-bold text-emerald-500/80 uppercase tracking-widest">Key Strengths</h5>
                                      </div>
                                     <div className="space-y-4">
  <div className="flex items-center gap-2">
    <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full" />
  ...
  <ul className="space-y-3">
    ...
    {(selectedCandidate.evaluation?.strengths || []).map((s: string, i: number) => (
      <li key={i} className="text-xs text-slate-400 bg-slate-800/20 p-3 rounded-xl border border-slate-700/20 flex items-start gap-3">
...
<div className="space-y-4">
  <div className="flex items-center gap-2">
    ...
    <h5 className="text-[10px] font-bold text-amber-500/80 uppercase tracking-widest">Areas for Improvement</h5>
  </div>
  <ul className="space-y-3">
    {/* ✅ ADDED ?. AND || [] SAFETY CHECKS */}
        ...
        <span>{w}</span>
                    ...
                    {!selectedCandidate.evaluation && (
                      ...
                      <div className="max-w-4xl mx-auto w-full space-y-6">
                        <div className="flex items-center justify-between">
                          <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Question Review</h4>
                          <button 
                          ...
                          >
                            {isSavingQuestions ? <Loader2 className="w-3 h-3 animate-spin" /> : <Save className="w-3 h-3" />}
                            Save Changes
                          </button>
                        </div>
                        ...
                        <div className="grid gap-4">
                          {editableQuestions.length === 0 ? (
                            <div className="p-12 bg-slate-800/20 rounded-3xl border border-dashed border-slate-700 text-center">
                              <p className="text-sm text-slate-500 italic">No questions generated yet.</p>
                            </div>
                          ) : (
                            editableQuestions.map((q, i) => (
                              <div key={i} className="p-4 bg-slate-800/30 rounded-2xl border border-slate-800 flex gap-4 group">
                                <span className="w-8 h-8 bg-blue-600/10 rounded-lg flex items-center justify-center text-xs font-bold text-blue-400 shrink-0">{i + 1}</span>
                                ...
                                <button 
                                  onClick={() => {
                                    const updated = editableQuestions.filter((_, idx) => idx !== i);
                                    setEditableQuestions(updated);
                                  }}
                        ...
                        <button 
                          onClick={() => setEditableQuestions([...editableQuestions, ""])}
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-12 text-center space-y-8">
                      ...
                      <div className="space-y-2">
                        <h3 className="text-xl font-bold text-slate-300">Select a Candidate</h3>
                        <p className="text-sm text-slate-500 max-w-xs mx-auto">Choose a candidate from the sidebar to manage their interview process.</p>
                      </div>
                    </div>
                )}
                ...
                {editingCandidate && (
                  ...
                  >
                    ...
                    >
                      <div className="p-6 border-b border-slate-800 flex items-center justify-between">
                        <div>
                          <h3 className="font-bold">Edit Questions for {editingCandidate.candidate_name}</h3>
                          <p className="text-xs text-slate-500">{editingCandidate.applied_role}</p>
                        </div>
                        <button onClick={() => setEditingCandidate(null)} className="p-2 hover:bg-slate-800 rounded-lg">
                          <X className="w-5 h-5" />
                        </button>
                      </div>
                      <div className="flex-1 overflow-y-auto p-6 space-y-4">
                          ...
                          <div key={idx} className="flex gap-2 group">
                            <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button 
                                onClick={() => {
                                  if (idx === 0) return;
                                  const updated = [...(editingCandidate.approved_questions || [])];
                                  [updated[idx-1], updated[idx]] = [updated[idx], updated[idx-1]];
                                  setEditingCandidate({ ...editingCandidate, approved_questions: updated });
                                }}
                              ...
                              <button 
                                onClick={() => {
                                  if (idx === (editingCandidate.approved_questions?.length || 0) - 1) return;
                                  const updated = [...(editingCandidate.approved_questions || [])];
                                  [updated[idx+1], updated[idx]] = [updated[idx], updated[idx+1]];
                                  setEditingCandidate({ ...editingCandidate, approved_questions: updated });
                                }}
                                className="p-1 hover:bg-slate-800 rounded text-slate-500 hover:text-blue-400 disabled:opacity-20"
                                disabled={idx === (editingCandidate.approved_questions?.length || 0) - 1}
                              >
                        ...
                        <button 
                          onClick={() => {
                            const updated = [...(editingCandidate.approved_questions || []), "New Question"];
                            setEditingCandidate({ ...editingCandidate, approved_questions: updated });
                          }}
                      ...
                      <div className="p-6 border-t border-slate-800 bg-slate-900/50">
                        <button 
                          onClick={() => {
                            saveEditedQuestions(editingCandidate.id!, editingCandidate.approved_questions!);
                            setEditingCandidate(null);
                          }}
          ...
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-8 space-y-6 shadow-2xl">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-bold flex items-center gap-2">
                <Share2 className="w-5 h-5 text-blue-400" /> Share Settings
              </h3>
              <button onClick={() => setShowShareSettings(false)} className="text-slate-500 hover:text-white">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-xs font-medium text-slate-400 ml-1">Base Share URL</label>
                <input 
                  type="text"
                  value={baseShareUrl}
                ...
                <p className="text-[10px] text-slate-500 leading-relaxed">
                  Genie uses this URL to generate candidate interview links. By default, it uses the current origin. 
                  If you are using the AI Studio "Share" link, paste it here to ensure candidates are directed correctly.
                </p>
              </div>
              ...
              <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl space-y-2">
                ...
                <p className="text-[10px] text-slate-400 leading-relaxed">
                  If candidates see a 404 error, ensure this URL is exactly what you see in the AI Studio "Share" button.
                </p>
              </div>
            </div>
      ...
      {/* Evaluation Modal */}
              ...
              >
                <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-900/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-lg">Interview Evaluation</h3>
                      <p className="text-xs text-slate-500">{showEvaluation.candidate_name} • {showEvaluation.applied_role}</p>
                    </div>
                  </div>
                  <button 
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="md:col-span-2 space-y-4">
                      <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Overall Summary</h4>
                      <p className="text-slate-300 leading-relaxed">{showEvaluation.overall_summary}</p>
                    </div>
                    <div className="bg-slate-800/50 rounded-2xl p-6 border border-slate-700/50 flex flex-col items-center justify-center text-center space-y-3">
                      <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Recommendation</h4>
                      <div className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider ${
                        showEvaluation.hiring_recommendation === 'strong_hire' ? 'bg-green-500/20 text-green-500 border border-green-500/30' :
                        showEvaluation.hiring_recommendation === 'hire' ? 'bg-blue-500/20 text-blue-500 border border-blue-500/30' :
                        showEvaluation.hiring_recommendation === 'borderline' ? 'bg-yellow-500/20 text-yellow-500 border border-yellow-500/30' :
                        'bg-red-500/20 text-red-500 border border-red-500/30'
                      }`}>
                        {showEvaluation.hiring_recommendation.replace('_', ' ')}
                      </div>
                      <p className="text-[10px] text-slate-500 mt-2">Next Step: {showEvaluation.recommended_next_step}</p>
                    </div>
                  </div>

                  {/* Scores Grid */}
                  <div className="space-y-4">
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Performance Scores</h4>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {Object.entries(showEvaluation.scores).map(([key, value]: [string, any]) => (
                        <div key={key} className="bg-slate-800/30 rounded-xl p-4 border border-slate-800/50">
                          <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">{key.replace(/_/g, ' ')}</div>
                          <div className="flex items-end gap-1">
                            <span className="text-2xl font-bold text-slate-200">{value}</span>
                            <span className="text-[10px] text-slate-600 mb-1.5">/ 10</span>
                          </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      ...
                      <ul className="space-y-2">
                        {showEvaluation.strengths.map((s: string, i: number) => (
                    ...
                    <div className="space-y-4">
                      ...
                      <ul className="space-y-2">
                        {showEvaluation.weaknesses.map((w: string, i: number) => (
                          <li key={i} className="flex gap-3 text-sm text-slate-300">
                  ...
                  {/* Question Analysis */}
                  <div className="space-y-4">
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Question-by-Question Analysis</h4>
                    <div className="space-y-4">
                      {showEvaluation.question_by_question_analysis.map((item: any, i: number) => (
                        <div key={i} className="bg-slate-800/20 rounded-2xl p-6 border border-slate-800/50 space-y-3">
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Question {i + 1}</div>
                            ...
                            <div className="flex flex-col items-end">
                              ...
                              <div className="text-lg font-bold text-slate-300">{item.score}/10</div>
                            </div>
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3 border-t border-slate-800/50">
                            <div>
                              <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Answer Quality</div>
                              <p className="text-xs text-slate-400 italic">"{item.answer_quality}"</p>
                            </div>
                            <div>
                              <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Key Observation</div>
                              <p className="text-xs text-slate-400">{item.key_observation}</p>
                            </div>
                          </div>
                        </div>
                      ))}
        ...
        <canvas ref={canvasRef} className="hidden" />
            ...
            <div className="p-12 flex flex-col h-full overflow-y-auto">
              <div className="max-w-xl mx-auto w-full space-y-8">
                <div className="space-y-2">
                  <h2 className="text-2xl font-bold">Prepare Candidate Interview</h2>
                  <p className="text-slate-400 text-sm">InterviewGenie Recruiter Assistant will prepare a personalized session.</p>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase">Applied Role</label>
                    <input 
                      value={newCandidate.applied_role}
                      onChange={e => setNewCandidate({ ...newCandidate, applied_role: e.target.value })}
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea 
                      ...
                      onChange={e => setNewCandidate({ ...newCandidate, job_description: e.target.value })}
                  ...
                  {newCandidate.generatedQuestions.length === 0 ? (
                    <button
                      ...
                      disabled={isGenerating || !newCandidate.candidate_name || !newCandidate.applied_role}
                      className="w-full flex items-center justify-center gap-3 px-8 py-4 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 rounded-2xl font-bold transition-all"
                    >
                      {isGenerating ? (
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-500 uppercase">Review Draft Questions</label>
                        <div className="space-y-2">
                          {newCandidate.generatedQuestions.map((q, i) => (
                            <input 
                              ...
                              value={q}
                              onChange={e => {
                                const updated = [...newCandidate.generatedQuestions];
                                updated[i] = e.target.value;
                                setNewCandidate({ ...newCandidate, generatedQuestions: updated });
                              }}
                              className="w-full bg-slate-800/50 border border-slate-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500"
                            />
                          ))}
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-3 px-8 py-4 bg-green-600 hover:bg-green-500 rounded-2xl font-bold transition-all"
                      >
                        <CheckCircle2 className="w-5 h-5" /> Approve & Save Candidate
                      </button>
                    </div>
                  )}
                </div>
              ...
              <div className="p-8 border-b border-slate-800 flex items-center justify-between bg-slate-900/30 shrink-0">
                <div className="flex-1 min-w-0">
                  <h2 className="text-xl font-bold truncate">
                    {appliedRole || (selectedCandidate ? selectedCandidate.applied_role : "") || "Interview Session"}
                  </h2>
                  <div className="flex items-center gap-4 mt-1">
                    {!isConnected ? (
                      <div className="flex items-center gap-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Candidate:</label>
                        <input 
                          value={candidateName}
                      ...
                      <div className="flex items-center gap-4">
                        <p className="text-slate-400 text-xs">Candidate: {candidateName}</p>
                        <div className="h-4 w-px bg-slate-800" />
                        <p className="text-blue-400 text-xs font-bold uppercase tracking-wider">Question {Math.min(questionCount + 1, 10)} of 10</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {fetchError === 'invalid_link' ? (
                  ...
                  <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-3xl p-8 text-center space-y-6">
                    ...
                    {!questionsReady && currentInterviewId ? (
                      <div className="space-y-4">
                        <div className="space-y-2">
                          <h3 className="text-xl font-bold">InterviewGenie is not ready yet</h3>
                          <p className="text-sm text-slate-400">
                            The recruiter is still preparing your interview questions. Please check back later or contact your recruiter.
                          </p>
                        </div>
                        <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                          <p className="text-xs text-blue-400 font-medium">Status: Awaiting Question Approval</p>
                        </div>
                      </div>
                    ) : (
                      <>
                        <div className="space-y-4">
                          <div className="space-y-1">
                            <h3 className="text-xl font-bold">Join Interview</h3>
                            <p className="text-sm text-slate-400">
                              {appliedRole || "Enter your Interview ID or Email to join"}
                            </p>
                            {candidateName && (
                          ...
                          <div className="space-y-2 text-left">
                            <label className="text-xs font-medium text-slate-400 ml-1">Interview ID or Email</label>
                            <input
                              type="text"
                          ...
                          {joinError && (
                            <p className="text-xs text-red-400 bg-red-400/10 py-2 rounded-lg">{joinError}</p>
                          )}
                        </div>
                    ...
                    <div className="pt-4 border-t border-slate-800">
                      <p className="text-[10px] text-slate-500">
                        By joining, you agree to the interview recording and transcription.
                      </p>
                    </div>
                  </div>
                ) : !user && view !== 'candidate' ? (
                  <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-3xl p-8 text-center space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-slate-400">Please sign in with your recruiter account to manage interviews.</p>
                    </div>
                    <button
                      onClick={login}
                    ...
                    {isCompleted ? (
                      <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-3xl p-8 text-center space-y-6">
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="text-sm text-slate-400">Thank you for your time. The interview has ended.</p>
                          <p className="text-sm text-slate-400">Your responses will be reviewed by the recruiter.</p>
                        </div>
                        {isRecruiter(user) && (
                          <button 
                            onClick={() => {
